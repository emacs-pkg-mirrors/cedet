...
;; Created: 10 Nov 2000
;; Keywords: syntax
;; X-RCS: $Id: senator.el,v 1.52 2001/11/14 19:42:36 ponced Exp $
...
(defvar senator-last-local-item-name  "_ In this buffer")
(defvar senator-last-global-item-name "_ Globally")

(defsubst senator-new-menu-item-name (&optional global)
  "Return a new unique generated menu item name.
If optional GLOBAL is non-nil generate a menu item name for a global
setting else generate one for a local setting."
  (if global
      (prog1
          senator-last-global-item-name
        (setq senator-last-global-item-name
              (concat senator-last-global-item-name " ")))
    (prog1
        senator-last-local-item-name
      (setq senator-last-local-item-name
            (concat senator-last-local-item-name " ")))))

(defvar senator-registered-mode-entries  nil)
(defvar senator-registered-mode-settings nil)

(defun senator-register-mode-menu-item (spec global &optional name)
...
local setting.  If optional argument NAME is given it must be a string
that will be used as the menu item name.  By default, a menu item name
is generated by `senator-new-menu-item-name'.
...
is a symbol which refer to a bound variable too."
  (and (consp spec)
       ...
       (let* ((callback (car spec))     ; callback function
         ...
         (or name (setq name (senator-new-menu-item-name global)))
...
and GLOBAL respectively define a menu item for local and global
settings.  They can be nil to omit the corresponding menu item.  See
the function `senator-register-mode-menu-item' for the menu item
specification.  If NAME is already registered the corresponding entry
will be updated with the given LOCAL and GLOBAL definitions.

Each registered minor mode will be displayed like this:

      NAME
  [x] _ In this buffer
  [x] _ Globally

depending on values of LOCAL (In this buffer) and GLOBAL (Globally)."
  (let* ((entry (assoc name senator-registered-mode-entries))
         (local-item       (cadr entry))
         (global-item      (cddr entry))
         (local-item-name  (if local-item  (aref local-item  0)))
         (global-item-name (if global-item (aref global-item 0))))
    (setq local-item  (senator-register-mode-menu-item
                       local nil local-item-name)
          global-item (senator-register-mode-menu-item
                       global t global-item-name))
...
(defun senator-build-mode-menu-items (entry)
  ...
  (let ((name   (car entry))
    ...
    (if window-system
      ...
      (if local
          (aset local  0 (concat name " (In this buffer)")))
      (if global
          (aset global 0 (concat name " (Globally)")))
      (setq name nil))
